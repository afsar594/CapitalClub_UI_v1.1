...
export class LeaveapprovalComponent implements OnInit, AfterViewInit {
  ...
  const fromDateValue = this.leaveform.get('fromdate').value;
  if (fromDateValue) {
    ...
    if (fromDate < today) {
      this.messageService.add({ severity: 'error', summary: 'Invalid Date', detail: 'Cannot approve leave for a past start date' });
      return;
    }
  }
  ...
  this.LeaveDataService.ApproveLeaveData(this.leaveid, 2, this.currentUserData.id, this.approveRemarks).subscribe(
    res => {
      if (res == null) {
...
// }
  ...
  GetSavedLeavedata(id: number) {
    ...
    onLeaveChange(name) {
  if (name.leaveType !== 'Annual Leave' && name.leaveType !== 'Sick Leave') {
    return;
  } else {
