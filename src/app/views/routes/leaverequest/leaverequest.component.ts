...
export class LeaverequestComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.serviceReqService
      .ReadStaffDatas(this.currentUserData.id)
      .subscribe((res) => {
        this.empdata = res;
      });
  ...
  onSubmit(leavdata: NewLeaveDataModel) {
    ...
    leavdata.TotalDays = this.gettotalDaysWithoutLeave();
    leavdata.imageFile = this.uploadedFiles;
    if (!this.leaveform.valid) {
      this.messageService.add({
        severity: 'error',
        summary: 'Cant Process Now',
    ...
    if (leavdata.TotalDays == 0) {
      this.messageService.add({
        severity: 'error',
        summary: 'Cant Process Now',
        detail: 'Days should be grater than 0',
      });
      return false;
    }
    leavdata.EmployeeID = this.currentUserData.id;
    this.submitted = true;
    this.LeaveDataService.SendNewLeaveRequest(leavdata).subscribe(
      (res) => {
        this.messageService.add({
          severity: 'success',
          summary: 'Success',
          detail: 'Successfull Added',
        });
  ...
  onLeaveChange(event) {
    ...
    if (this.totalDOJDays <= 90) {
      if (
        event.value.leaveType === 'Sick Leave' ||
        event.value.leaveType === 'Annual Leave'
      ) {
        if (!this.isload) {
          this.messageService.add({
            severity: 'error',
  ...
  getLeaveTypesById() {
    this.leaveTypeAService
      .getLeaveTypeById(Number(this.currentUserData.id))
      .subscribe((res: any) => {
  ...
  gettotalDaysWithoutLeave() {
    return this.totalDays - this.totalholidays;
  }
  ...
  get diagnostic() {
    return JSON.stringify(this.leaveform.value);
  }
}
